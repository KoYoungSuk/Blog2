...
<body>
 ...
 <H3 class="htitle" id="htitle_id"> DailyManager (Web) </H3>
 <hr>
 ...
 <table style="margin: auto;">
   <tr>
    <td>
     <input type="text" name="title" class="form form-control" placeholder="Input Title Here" /> 
    </td>
    <td>
 ...
 <H5 style="font-weight: bold; ">DailyManager(Web) is Administrator Only! </H5>
 </div> 
 <hr> 
 <div id="calendar" style="text-align: center;"></div>
 <div style="text-align: center;">
 <H5 style="font-weight: bold;">${sessionScope.errormessage_daily}</H5>
 <c:choose>
  <c:when test="${sessionScope.errormessage_daily ne null}">
   <c:choose> 
   ...
   <button type="button" onclick="location.href='daily.jsp?page=3&title=${param.choosed_title}'" class="btn btn-secondary btn-sm"><span class="material-symbols-outlined">create</span>Write</button>
  </c:when>
  <c:otherwise></c:otherwise> 
 </c:choose> 
 </div>
					...
					if (calendarMonthStartDay <= calendarPos && calendarDay < calendarMonthLastDate) { //마지막 일까지만 출력&위치 맞추기 	
						...
						if(calendarDay < 10){ //10일 이하일때 
							if(calendarMonth < 10){ //1~9월 
								html += "<a href= \"detaildaily?title=" + calendarYear + "-0" + calendarMonth + "-0" + calendarDay +  " \">";
							}
							else{ //10~12월 
								html += "<a href= \"detaildaily?title=" + calendarYear + "-" + calendarMonth + "-0" + calendarDay +  " \">";
							}
						}
						else{
							if(calendarMonth < 10){ //1~9월 
								html += "<a href= \"detaildaily?title=" + calendarYear + "-0" + calendarMonth + "-" + calendarDay +  " \">";
							}
							else{ //10~12월 
								html += "<a href= \"detaildaily?title=" + calendarYear + "-" + calendarMonth + "-" + calendarDay +  " \">";
							}
						}
						html += "<span "; 
						//현재 날짜일때 빨간색으로 바꾸기 
